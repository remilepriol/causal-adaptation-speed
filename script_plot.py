...
def optim_plot():
    ...
    for exp in results:
        ...
        longplot(exp, colors, statistics=True, plot_bound=False)
        plt.savefig(os.path.join(plotdir, 'average_' + longplotname(exp)))
        # longplot(exp, colors, statistics=False)
        # plt.savefig(os.path.join(plotdir, 'curves_' + longplotname(exp)))
        optim_scatter(exp, colors)
        plt.savefig(os.path.join(plotdir, 'scatter_' + longplotname(exp)))
        plt.close()


def longplot(exp, colors, confidence=(5, 95), statistics=True, plot_bound=False):
    ...
    if plot_bound:
        for model_family in ['causal', 'anti']:
            ...
            print(f"{model_family} initial distance = {initial_distance:.2f} \t"
                  f" bound constant = {constant:.2f} \t ratio = {constant / initial_distance:.2f} \t"
                  f"ratio constant / initial kl = {constant / exp['kl_' + model_family][0].mean():.2f}")

            axs[0].plot(steps, bound, color=colors[model_family], linestyle='--')


def longplotname(exp):
    return ('{intervention}_k={k}_bs={batch_size}_rate={scheduler_exponent:.1f}'
...
def rate_constant_fullbatch(smoothness, initial_lr, initial_distance):
    return - initial_distance / (initial_lr * (smoothness / 2 * initial_lr - 1))


def optim_scatter(exp, colors, end_step=1000):
    fig, axs = plt.subplots(nrows=2, ncols=1, sharex=True, figsize=(10, 8))
    ...
    ends = {}
    integrals = {}
    for model_family in ['anti', 'causal']:
        initial_distances[model_family] = exp['scoredist_' + model_family][0]
    ...
    for ax,  metrics in zip(axs, [ends, integrals]):
        ...
        ax.set_yscale('log')
        ax.set_xscale('log')
        ax.plot(np.stack((initial_distances['causal'], initial_distances['anti']), axis=1).T,
                np.stack((metrics['causal'], metrics['anti']), axis=1).T,
                alpha=.1, color='black')
    ...
    axs[1].set_xlabel('Parameter squared distance from initialization to optimum.')
